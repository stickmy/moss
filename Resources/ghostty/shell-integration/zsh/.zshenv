...
if [[ -n "${MOSS_DEBUG_LOG_PATH:-}" ]]; then
    'builtin' 'print' -r -- "[moss][shell][zshenv][pid=$$] sourced interactive=${options[interactive]:-off} zdotdir=${ZDOTDIR:-} shell=${ZSH_NAME:-zsh}" >> "${MOSS_DEBUG_LOG_PATH}" 2>/dev/null
fi
...
# Use try-always to have the right error code.
{
    ...
    if [[ -o 'interactive' ]]; then
        ...
        if [[ -n "${MOSS_DEBUG_LOG_PATH:-}" ]]; then
            'builtin' 'print' -r -- "[moss][shell][zshenv][pid=$$] interactive ghostty_file=${_ghostty_file}" >> "${MOSS_DEBUG_LOG_PATH}" 2>/dev/null
        fi
